...
const RC2_METHOD_HASHES = Object.freeze({
...
export function compatibilityMethodHashes(prototype) {
  const result = {}
  for (const name of Object.keys(RC2_METHOD_HASHES)) {
    const method = prototype?.[name]
    if (typeof method !== 'function') return null
    result[name] = createHash('sha256').update(Function.prototype.toString.call(method)).digest('hex')
  }
  return result
}

export function matchesRc2CompatibilityRegistry(registry) {
  const hashes = compatibilityMethodHashes(registry?.constructor?.prototype)
  return hashes !== null
    && Object.entries(RC2_METHOD_HASHES).every(([name, expected]) => hashes[name] === expected)
}

/**
 * A future public method wins. The private fallback is allowed only when the
 * exact rc.2 method implementations and the live state shape both match.
 */
export function inspectRestoreCapability(registry, compatibilityCheck = matchesRc2CompatibilityRegistry) {
  ...
  return {
    canRestore: false,
    mode: 'unsupported',
    message: '当前 DSH 未提供官方恢复接口，且不匹配已验证的 rc.2 兼容指纹；恢复已安全禁用。',
  }
}
...
async function restoreWithCompatibilityAdapter(registry, sessionId, compatibilityCheck) {
  if (!compatibilityCheck(registry) || typeof registry?.enqueueOperation !== 'function') {
    throw new PublicArchiveError('restore-adapter-unavailable', 'rc.2 兼容指纹已变化；恢复已安全停止', 409)
  }
...
export function createArchiveManager({
  workspaceRegistry,
  sessionQuery,
  compatibilityCheck = matchesRc2CompatibilityRegistry,
}) {
