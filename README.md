...
`verify:runtime` 会核对声明的 DSH 版本、rc.2 适配器方法指纹，以及当前 Settings、Sidebar、Overlay、Session Query、Web Server 类型契约。它失败时不得绕过或手工放宽指纹；应先审查新版本并发布插件更新。即使自动检查通过，GUI 验证仍是必需步骤。
...
3. 管理页能列出归档会话；
4. 页面顶部的兼容能力提示符合预期；
5. 恢复一个测试会话后，它重新出现在原工作区位置；
6. 页面刷新和 DSH 重启后恢复结果仍然存在。
