...
- 在 DSH 现有设置面板中增加“已归档会话”页面，并直接展示全部已归档会话；
...
插件只使用公开的 `settings.section` Slot 提供完整设置页，不修改 Web Shell、不替换顶层页面、不依赖内部 DOM，也不劫持浏览器 history。
...
`verify:runtime` 会核对声明的 DSH 版本、rc.2 适配器方法指纹，以及当前 Settings、Session Query、Web Server 类型契约。它失败时不得绕过或手工放宽指纹；应先审查新版本并发布插件更新。即使自动检查通过，GUI 验证仍是必需步骤。

随后在 GUI 中确认：

1. 设置页入口仍可见，且侧栏底部不出现插件入口；
2. 打开“已归档会话”设置页后，归档列表直接显示且刷新可用；
